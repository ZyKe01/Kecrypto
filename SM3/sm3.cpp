...
void sm3_init2(sm3_ctx_t* ctx, unsigned char* IV);
void sm3_update(sm3_ctx_t* ctx, const unsigned char* data, size_t data_len);
void sm3_final(sm3_ctx_t* ctx, unsigned char digest[SM3_DIGEST_LENGTH], bool flag);
...
int main(int argc, char* argv[])
{
	...
	sm3(str1, length, out2);
	print_str(out2, 32);

	printf("-----------------------------\n");
	lenExAtt();
	/*unsigned char tmp[4] = { 0x01,0x02,0x03,0x04 };
	uint32_t aa;
	memcpy(&aa, tmp, 4);
	printf("%x\n", aa);*/
	return 0;
...
void sm3_final(sm3_ctx_t* ctx, unsigned char* digest, bool flag)
{
	int i;
	...
	if (flag) {
		...
		memset(ctx->block + ctx->num, 0, SM3_BLOCK_SIZE - ctx->num);
	}
